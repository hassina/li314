...
public class Labyrinthe implements Serializable {
	...
	/**
	 * Crée un labyrinthe cerné de murs et couverts de points, des dimensions
	 * indiquées.
	 * 
	 * @param width
	 *            : nombre de colonnes
	...
	public boolean estEntoureDeMurs() {
		...
		for (int i = 0; i < Xsize(); i++) {
			if (carte[i][jmax] != ContenuCase.MUR)
				return false;
			if (carte[i][0] != ContenuCase.MUR)
				return false;
		}
		for (int j = 0; j < Ysize(); j++) {
			if (carte[0][j] != ContenuCase.MUR)
				return false;
			if (carte[imax][j] != ContenuCase.MUR)
				return false;
		}
		return true;
	}
