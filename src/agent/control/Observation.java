...
public class Observation {
	 ...
	 * les cases perçues dans le labyrinthe. Les cases qui débordent du
	 * labyrinthe sont remplies de ANY.
	 * 
	...
	public Observation(Point position, Labyrinthe laby) {
		capteurs[0] = laby.getContenuCase(position.x, position.y - 1);
		capteurs[1] = laby.getContenuCase(position.x + 1, position.y - 1);
		capteurs[2] = laby.getContenuCase(position.x + 1, position.y);
		capteurs[3] = laby.getContenuCase(position.x + 1, position.y + 1);
		capteurs[4] = laby.getContenuCase(position.x, position.y + 1);
		capteurs[5] = laby.getContenuCase(position.x - 1, position.y + 1);
		capteurs[6] = laby.getContenuCase(position.x - 1, position.y);
		capteurs[7] = laby.getContenuCase(position.x - 1, position.y - 1);
	}

	/**
	 * Crée une Observation basée sur une chaine de caractères composée de 8
	 * caractère : # : mur, ? : ANY, . : point, ' ' : vide, A : Agent
	 * 
	 * Les caractères sont interprétés dans la convention horaire: 0 = Nord, 1 =
	 * NE, 2 = E, 3 = SE, 4 = S, 5 = SO, 6 = O, 7 = NO
	 * 
	 * @param contenu
	 */
	 ...
	 * 
	 * @param capteurs
	 *            tableau de capteurs à copier dans l'objet.
	 */
